//
//  SWPasterWonderlandViewController.h
//  SketchWonderLand
//
//  Created by  on 12-3-20.
//  Copyright 2012年 __MyCompanyName__. All rights reserved.
//

#import <UIKit/UIKit.h>

@interface SWPasterWonderlandViewController : UIViewController{
    UIImageView *backgroundImageView;
    UIButton *pasterTemplateButton1;
    UIButton *pasterTemplateButton2;
    UIButton *pasterTemplateButton3;
    UIButton *pasterTemplateButton4;
    UIButton *pasterTemplateButton5;
    UIButton *pasterTemplateButton6;
    UIButton *returnButton;
    
}

@property (nonatomic, retain) IBOutlet UIImageView *backgroundImageView;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton1;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton2;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton3;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton4;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton5;
@property (nonatomic, retain) IBOutlet UIButton *pasterTemplateButton6;
@property (nonatomic, retain) IBOutlet UIButton *returnButton;


@end
